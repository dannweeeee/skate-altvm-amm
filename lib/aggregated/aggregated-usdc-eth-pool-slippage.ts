...
export async function getAggregatedUsdcEthPoolSlippage(
  ...
  const ethereumParams = {
    tokenIn: ETHEREUM_USDC_TOKEN.address,
  ...
  const baseParams = {
    tokenIn: BASE_USDC_TOKEN.address,
  ...
  const optimismParams = {
    tokenIn: OPTIMISM_USDC_TOKEN.address,
    tokenOut: OPTIMISM_WETH_TOKEN.address,
    fee: 500,
  ...
  const arbitrumParams = {
    tokenIn: ARBITRUM_USDC_TOKEN.address,
  ...
  const price = sqrtToPrice(aggregatedSqrtPriceX96, 6, 18, false);
  const priceAfter = sqrtToPrice(aggregatedSqrtPriceX96After, 6, 18, false);
  ...
  // this is the price impact in the scenario where the amountIn is split between 4 pools equally
