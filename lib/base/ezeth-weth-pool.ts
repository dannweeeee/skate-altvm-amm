...
import { BaseEzethWethConfig } from "../../config";
import { BarChartTick, GraphTick } from "../interfaces";
import { createBarChartTicks } from "../active-liquidity";
import { getBuiltGraphSDK } from "../../.graphclient";

export async function getBaseEzethWethPool(): Promise<{
  pool: Pool;
  baseTicks: BarChartTick[];
}> {
  console.log("Starting Base ezETH WETH getBaseEzethWethPool function");

  const poolAddress = BASE_EZETH_WETH_POOL_CONTRACT_ADDRESS;
  ...
  const fullPool = new Pool(
    BaseEzethWethConfig.pool.tokenA,
    BaseEzethWethConfig.pool.tokenB,
    BaseEzethWethConfig.pool.fee,
    slot0.sqrtPriceX96,
    liquidity,
    slot0.tick,
    sdkTicks
  ...
  console.log("BASE ezETH WETH Pool activeTickIdx", activeTickIdx);
  ...
  const barChartTicks = await createBarChartTicks(
    ...
    fullPool.token0,
    fullPool.token1,
    BaseEzethWethConfig.chart.numSurroundingTicks,
    fullPool.fee,
    graphTicks
  );

  console.log("Returning Base ezETH WETH getBaseEzethWethPool function");
