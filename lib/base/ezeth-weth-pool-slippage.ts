...
import {
  ...
  BASE_UNISWAP_V3_FACTORY_CONTRACT_ADDRESS,
  BASE_WETH_TOKEN,
} from "./constants";
...
const getAbi = (address: string) =>
  address.toLowerCase() === BASE_WETH_TOKEN.address.toLowerCase()
    ? WETH_ABI
    : ERC20_ABI;
...
export async function getBaseEzethWethPoolSlippage(
  tokenIn: any,
  tokenOut: any,
  fee: any,
  amountIn: any
  ...
  console.log("absolutePriceImpact", absolutePriceImpact); // doesnt account of added liquidity in the same block
