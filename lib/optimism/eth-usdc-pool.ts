...
import { OPTIMISM_ETH_USDC_POOL_CONTRACT_ADDRESS } from "../optimism/constants";
import { UNISWAP_V3_POOL_CONTRACT_ABI } from "../../abis/uniswapv3-pool-abi";
import { getOptimismProvider } from "../providers";
import { OptimismEthUsdcConfig } from "../../config";
import { BarChartTick, GraphTick } from "../interfaces";
import { createBarChartTicks } from "../active-liquidity";
import { getBuiltGraphSDK } from "../../.graphclient";

export async function getOptimismEthUsdcPool(): Promise<{
  pool: Pool;
  optimismTicks: BarChartTick[];
}> {
  console.log("Starting Optimism ETH USDC getOptimismEthUsdcPool function");

  const poolAddress = OPTIMISM_ETH_USDC_POOL_CONTRACT_ADDRESS;
  ...
  const fullPool = new Pool(
    OptimismEthUsdcConfig.pool.tokenA,
    OptimismEthUsdcConfig.pool.tokenB,
    OptimismEthUsdcConfig.pool.fee,
    slot0.sqrtPriceX96,
    liquidity,
    slot0.tick,
    sdkTicks
  ...
  console.log("OPTIMISM ETH USDC Pool activeTickIdx", activeTickIdx);
  ...
  const barChartTicks = await createBarChartTicks(
    ...
    fullPool.token0,
    fullPool.token1,
    OptimismEthUsdcConfig.chart.numSurroundingTicks,
    fullPool.fee,
    graphTicks
  );

  console.log("Returning Optimism ETH USDC getOptimismEthUsdcPool function");
