...
const EthereumUsdcEthSwapSimulation = () => {
  const [ethereumUsdcEthResult, setEthereumUsdcEthResult] =
    useState<SlippageResult | null>(null);
  const [ethereumEthUsdcResult, setEthereumEthUsdcResult] =
    useState<SlippageResult | null>(null);
  ...
  const handleSwapUsdcEth = () => {
    ...
    setEthereumUsdcEthResult(null);
    setEthereumEthUsdcResult(null);
    setEthAmountIn("");
    setUsdcAmountIn("");
  };
    ...
    try {
      if (swapUsdcEth) {
        if (usdcAmountIn) {
          try {
            const response = await fetch(
              `/api/ethereum/getEthereumUsdcEthSlippage?amountIn=${usdcAmountIn}`
            );
            const contentType = response.headers.get("content-type");
            if (contentType && contentType.includes("application/json")) {
              const data = await response.json();
              if (response.ok) {
                setEthereumUsdcEthResult(data.result);
                console.log("Ethereum USDC-ETH Slippage Result:", data.result);
              } else {
                console.log(
                  "Error fetching Ethereum USDC-ETH Slippage data:",
                  data
                );
              }
            }
        ...
        if (ethAmountIn) {
          try {
            const response = await fetch(
              `/api/ethereum/getEthereumEthUsdcSlippage?amountIn=${ethAmountIn}`
            );
            const contentType = response.headers.get("content-type");
            ...
            if (contentType && contentType.includes("application/json")) {
              const data = await response.json();
              if (response.ok) {
                setEthereumEthUsdcResult(data.result);
                console.log("Ethereum ETH-USDC Slippage Result:", data.result);
              } else {
                console.log(
                  "Error fetching Ethereum ETH-USDC Slippage data:",
                  data
                );
              }
            } else {
              console.error(
                "Unexpected response from Ethereum ETH-USDC Slippage API:",
                await response.text()
              );
            }
          } catch (error) {
    ...
    <div className="flex flex-col gap-5 items-center justify-center">
      ...
      {loading ? (
        ...
        <div className="text-left w-full text-sm sm:text-base">
          {swapUsdcEth
            ? ethereumUsdcEthResult && (
                <>
                  <p>
                    <span className="font-semibold">ETH Current Price:</span>{" "}
                    {ethereumUsdcEthResult?.price?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">ETH Price After Swap:</span>{" "}
                    {ethereumUsdcEthResult?.priceAfter?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">Slippage:</span>{" "}
                    {(ethereumUsdcEthResult?.slippage * -1)?.toFixed(6)}%
                  </p>
                </>
              )
            : ethereumEthUsdcResult && (
                <>
                  <p>
                    <span className="font-semibold">ETH Current Price:</span>{" "}
                    {(ethereumEthUsdcResult?.price * 1e24)?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">ETH Price After Swap:</span>{" "}
                    {(ethereumEthUsdcResult?.priceAfter * 1e24)?.toFixed(6)}{" "}
                    USDC
                  </p>
                  <p>
                    <span className="font-semibold">Slippage:</span>{" "}
                    {ethereumEthUsdcResult?.slippage?.toFixed(6)}%
                  </p>
                </>
              )}
        </div>
...
export default EthereumUsdcEthSwapSimulation;
