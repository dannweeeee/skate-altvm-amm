...
const AggregatedEthUsdcSwapSimulation = () => {
  const [aggregatedUsdcEthResult, setAggregatedUsdcEthResult] =
    useState<SlippageResult | null>(null);
  const [aggregatedEthUsdcResult, setAggregatedEthUsdcResult] =
    useState<SlippageResult | null>(null);
  ...
  const handleSwapUsdcEth = () => {
    ...
    setAggregatedUsdcEthResult(null);
    setAggregatedEthUsdcResult(null);
    setEthAmountIn("");
    setUsdcAmountIn("");
  };
    ...
    try {
      if (swapEthUsdc) {
        if (ethAmountIn) {
          try {
            const response = await fetch(
              `/api/aggregated/getAggregatedEthUsdcSlippage?amountIn=${ethAmountIn}`
            );
            ...
            if (contentType && contentType.includes("application/json")) {
              ...
              if (response.ok) {
                setAggregatedEthUsdcResult(data.result);
                console.log(
                  "Aggregated ETH-USDC Slippage Result:",
                  data.result
                );
              } else {
                console.log(
                  "Error fetching Aggregated ETH-USDC Slippage data:",
                  data
                );
              }
            } else {
              console.error(
                "Unexpected response from Aggregated ETH-USDC Slippage API:",
                await response.text()
              );
            }
          } catch (error) {
        ...
        if (usdcAmountIn) {
          try {
            const response = await fetch(
              `/api/aggregated/getAggregatedUsdcEthSlippage?amountIn=${usdcAmountIn}`
            );
            const contentType = response.headers.get("content-type");
            if (contentType && contentType.includes("application/json")) {
              const data = await response.json();
              if (response.ok) {
                setAggregatedUsdcEthResult(data.result);
                console.log(
                  "Aggregated USDC-ETH Slippage Result:",
                  data.result
                );
              } else {
                console.log(
                  "Error fetching Aggregated USDC-ETH Slippage data:",
                  data
                );
              }
            }
    ...
    <div className="flex flex-col gap-5 items-center justify-center">
      ...
      {loading ? (
        ...
        <div className="text-left w-full text-sm sm:text-base">
          {swapEthUsdc
            ? aggregatedEthUsdcResult && (
                <>
                  <p>
                    <span className="font-semibold">ETH Current Price:</span>{" "}
                    {(aggregatedEthUsdcResult?.price * 1e192)?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">ETH Price After Swap:</span>{" "}
                    {(aggregatedEthUsdcResult?.priceAfter * 1e192)?.toFixed(6)}{" "}
                    USDC
                  </p>
                  <p>
                    <span className="font-semibold">Slippage:</span>{" "}
                    {aggregatedEthUsdcResult?.slippage?.toFixed(6)}%
                  </p>
                </>
              )
            : aggregatedUsdcEthResult && (
                <>
                  <p>
                    <span className="font-semibold">ETH Current Price:</span>{" "}
                    {(aggregatedUsdcEthResult?.price * 1e168)?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">ETH Price After Swap:</span>{" "}
                    {(aggregatedUsdcEthResult?.priceAfter * 1e168)?.toFixed(6)}{" "}
                    USDC
                  </p>
                  <p>
                    <span className="font-semibold">Slippage:</span>{" "}
                    {(aggregatedUsdcEthResult?.slippage * -1)?.toFixed(6)}%
                  </p>
                </>
              )}
        </div>
...
export default AggregatedEthUsdcSwapSimulation;
