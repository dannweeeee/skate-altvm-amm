...
const BaseEthUsdcSwapSimulation = () => {
  const [baseUsdcEthResult, setBaseUsdcEthResult] =
    useState<SlippageResult | null>(null);
  const [baseEthUsdcResult, setBaseEthUsdcResult] =
    useState<SlippageResult | null>(null);
  ...
  const handleSwapUsdcEth = () => {
    ...
    setBaseUsdcEthResult(null);
    setBaseEthUsdcResult(null);
    setEthAmountIn("");
    setUsdcAmountIn("");
  };
    ...
    try {
      if (swapEthUsdc) {
        if (ethAmountIn) {
          try {
            const response = await fetch(
              `/api/base/getBaseEthUsdcSlippage?amountIn=${ethAmountIn}`
            );
            ...
            if (contentType && contentType.includes("application/json")) {
              ...
              if (response.ok) {
                setBaseEthUsdcResult(data.result);
                console.log("Base ETH-USDC Slippage Result:", data.result);
              } else {
                console.log(
                  "Error fetching Base ETH-USDC Slippage data:",
                  data
                );
              }
            } else {
              console.error(
                "Unexpected response from Base ETH-USDC Slippage API:",
                await response.text()
              );
            }
          } catch (error) {
        ...
        if (usdcAmountIn) {
          try {
            const response = await fetch(
              `/api/base/getBaseUsdcEthSlippage?amountIn=${usdcAmountIn}`
            );
            const contentType = response.headers.get("content-type");
            if (contentType && contentType.includes("application/json")) {
              const data = await response.json();
              if (response.ok) {
                setBaseUsdcEthResult(data.result);
                console.log("Base USDC-ETH Slippage Result:", data.result);
              } else {
                console.log(
                  "Error fetching Base USDC-ETH Slippage data:",
                  data
                );
              }
            }
            ...
            ? baseEthUsdcResult && (
                <>
                  <p>
                    ...
                    {(1 / baseEthUsdcResult?.price)?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">ETH Price After Swap:</span>{" "}
                    {(1 / baseEthUsdcResult?.priceAfter)?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">Slippage:</span>{" "}
                    {(baseEthUsdcResult?.slippage * -1)?.toFixed(6)}%
                  </p>
                </>
              )
            : baseUsdcEthResult && (
                <>
                  <p>
                    <span className="font-semibold">ETH Current Price:</span>{" "}
                    {((1 / baseUsdcEthResult?.price) * 1e24)?.toFixed(6)} USDC
                  </p>
                  <p>
                    <span className="font-semibold">ETH Price After Swap:</span>{" "}
                    {((1 / baseUsdcEthResult?.priceAfter) * 1e24)?.toFixed(6)}{" "}
                    USDC
                  </p>
                  <p>
                    <span className="font-semibold">Slippage:</span>{" "}
                    {baseUsdcEthResult?.slippage?.toFixed(6)}%
                  </p>
                </>
              )}
        </div>
...
export default BaseEthUsdcSwapSimulation;
